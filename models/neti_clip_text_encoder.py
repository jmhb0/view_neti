...
class NeTICLIPTextTransformer(CLIPTextTransformer):
    ...
    def forward(
        ...
        ###########################
            ...
            (hidden_states, bypass_outputs_object, bypass_outputs_view,
             bypass_unconstrained_object, bypass_unconstrained_view,
             output_bass_alpha_object,
             output_bass_alpha_view) = self.embeddings(
        ...
        if bypass_outputs_view is not None:
            bypass_output = bypass_outputs_view
            ...
            if not bypass_unconstrained_view:
                bypass_output = bypass_output / bypass_output.norm(dim=1, keepdim=True) \
                                * existing_state.norm(dim=1, keepdim=True) # this step matches the norm.
                # new_state = existing_state + 0.2 * bypass_output
                new_state = existing_state + output_bass_alpha_view * bypass_output
            else:
                normalizing_term = last_hidden_state_with_bypass.norm(
                    dim=-1).mean(-1)  # (bs,)
                normalizing_term = normalizing_term.detach()
